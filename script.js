...
function getPrediction(route, mode, timeOfDay) {
  ...
  // Data points for this route + mode: trips paid with this app, standalone price
  // checks, and quotes for this app logged alongside trips paid with a different app
  const checks = (state.priceChecks || []).filter(c => c.route === route && c.mode === mode && c.price > 0);
  const quotePoints = state.trips
    .filter(t => t.route === route && t.mode !== mode && t.quotes && t.quotes[mode] > 0)
    .map(t => ({ date: t.date, time: t.time, price: t.quotes[mode] }));
  const base = [
  ...
  // Weather adjustment 8–12%, seeded by date+route+mode so it's stable per day
  let weatherAdj = 0, weatherNote = '';
  if (isRainy) {
    const seed = getRainSeed(route, mode);
    const factor = 0.08 + seededRand(seed) * 0.04;
    weatherAdj = avg * factor;
...
function renderCompareGraphs() {
  ...
  el.innerHTML = routes.map(route => {
    const datasets = APPS.map(app => {
      ...
      // Add quotes logged for this app on trips paid with a different app
      const quotePts = state.trips
        .filter(t => t.route === route && t.mode !== app && t.quotes && t.quotes[app] > 0)
        .map(t => ({ x: new Date(t.date + 'T' + (t.time||'00:00')).getTime(), y: t.quotes[app], predicted: false }));

      const allReal = [...realPts, ...checkPts, ...quotePts].sort((a,b) => a.x - b.x);
...
function getRainSeed(route, mode) {
  const dayStr = new Date().toISOString().split('T')[0]; // changes once per calendar day
  const str = dayStr + route + mode;
  let hash = 0;
...
// Fish tank lifecycle — one cycle = 20.25 commute hours (time to grow from 1→18 fish).
// Even cycles = growth phase (1→18), odd cycles = death phase (18→1).
// When a new growth cycle starts after a death phase, show the GTA reference.
function getFishCycleState(totalMins) {
  const totalHours = totalMins / 60;
  const CYCLE_HOURS = 20.25; // (18/4)^2 — hours at which fish count peaks at 18
  const cycleNumber  = Math.floor(totalHours / CYCLE_HOURS);
  const cycleProgress = totalHours % CYCLE_HOURS;
  const isDying = cycleNumber % 2 === 1;
  let fishCount;
  if (isDying) {
    fishCount = Math.max(1, Math.min(18, 19 - Math.round(Math.sqrt(cycleProgress) * 4)));
  } else {
    fishCount = Math.min(18, Math.max(1, Math.round(Math.sqrt(cycleProgress) * 4)));
  }
  return { fishCount, isDying, cycleNumber };
}

function getFishCaption(fishCount, isDying, cycleNumber) {
  // GTA San Andreas — CJ stepping out of the car at the start of the game
  if (!isDying && cycleNumber > 0 && fishCount === 1) {
    return "ah sh*t, here we go again";
  }

  if (isDying) {
    const dyingSpecial = {
      17: "one down. the rest are not worried yet.",
      13: "thirteen remain. still haunted.",
      12: "carbon-12 slowly decaying. as nature intended.",
      8:  "eight left. oxygen levels: increasingly personal.",
      7:  "seven survivors. the favourite number clings on.",
      3:  "three fish. a love triangle with nothing to lose.",
      2:  "two fish left. the awkward silence has returned.",
      1:  "just the one survivor. waiting for something."
    };
    if (dyingSpecial[fishCount]) return dyingSpecial[fishCount];
    if (fishCount >= 14) return "they're leaving one by one...";
    if (fishCount >= 9)  return "the tank grows quieter";
    if (fishCount >= 4)  return "spacious again, but at what cost";
    return "the last few hold on";
  }

  // Growth phase — special numbers
  const special = {
    1:  "vast ocean, zero social obligations",
    2:  "two fish, infinite awkward silence",
    3:  "a love triangle — underwater edition",
    6:  "hexagonal harmony achieved. the bees would approve.",
    7:  "statistically the world's favourite number. the fish concur.",
    8:  "oxygen has 8 protons. the irony is not lost on the fish.",
    11: "11 — the only prime that looks like two fish side by side",
    12: "carbon-12 achieved — the tank is now technically organic chemistry",
    13: "thirteen fish. the tank is legally haunted now.",
    17: "17 fish — prime, peculiar, and somehow exactly right",
    18: "maximum occupancy. the fish have formed a committee and are drafting bylaws."
  };
  if (special[fishCount]) return special[fishCount];

  if (fishCount <= 4)  return "where did everyone go?";
  if (fishCount <= 9)  return "getting a bit cosy in here";
  if (fishCount <= 13) return "rush hour in the tank";
  return "it's giving sardines";
}

function renderFishTank(totalMins) {
  const { fishCount, isDying } = getFishCycleState(totalMins);

  const fishSvg = (color) => `
    <svg viewBox="0 0 32 20" style="overflow:visible">
      <path d="M4 10C4 6 9 4 15 4C21 4 27 7 29 10C27 13 21 16 15 16C9 16 4 14 4 10Z" stroke="${color}" stroke-width="1.5" fill="none" stroke-linejoin="round"/>
      <path d="M4 10L0 6M4 10L0 14" stroke="${color}" stroke-width="1.5" stroke-linecap="round" fill="none"/>
      <circle cx="20" cy="8" r="1" fill="${color}"/>
    </svg>`;

  // Water level — drops during death phase
  const fillPct = Math.round((fishCount / 18) * 100);
  const waterColor = isDying
    ? 'linear-gradient(180deg, rgba(255,77,77,0.05) 0%, rgba(255,77,77,0.12) 100%)'
    : 'linear-gradient(180deg, rgba(53,241,200,0.06) 0%, rgba(53,241,200,0.14) 100%)';
  let html = `<div class="tank-water" style="height:${fillPct}%;background:${waterColor}"></div>`;

  // Fish — slightly dimmer during death phase
  const opacity = isDying ? '0.5' : '0.85';
  for (let i = 0; i < fishCount; i++) {
    const top      = Math.round(6 + seededRand(i * 1.7 + 1) * 40);
    const duration = (8 + seededRand(i * 3.3 + 2) * 11).toFixed(1);
    const delay    = (-seededRand(i * 5.1 + 3) * duration).toFixed(1);
    const w        = Math.round(18 + seededRand(i * 2.1 + 4) * 12);
    const h        = Math.round(w * 0.6);
    const goingLeft = seededRand(i * 7.7 + 5) > 0.5;
    const color    = isDying
      ? (i % 2 === 0 ? 'var(--danger)' : 'var(--accent2)')
      : (i % 2 === 0 ? 'var(--accent)' : 'var(--accent2)');
    const anim = goingLeft ? 'swim-left' : 'swim-right';
    const flip = goingLeft ? 'transform:scaleX(-1)' : '';
    html += `
      <div class="swim-fish" style="top:${top}px;width:${w}px;height:${h}px;opacity:${opacity};animation:${anim} ${duration}s linear infinite;animation-delay:${delay}s;${flip}">
        ${fishSvg(color)}
      </div>`;
  }
  ...
  // Oxygen bubbles
  for (let b = 0; b < 5; b++) {
    const left  = Math.round(8 + seededRand(b * 4.1 + 99) * 84);
    const size  = (2 + seededRand(b * 2.3 + 77) * 3).toFixed(1);
    const dur   = (3 + seededRand(b * 1.9 + 55) * 4).toFixed(1);
    const dly   = (seededRand(b * 3.7 + 33) * dur).toFixed(1);
    const color = b % 2 === 0 ? 'var(--accent2)' : 'var(--accent)';
    html += `<div class="tank-bubble" style="left:${left}%;width:${size}px;height:${size}px;animation-duration:${dur}s;animation-delay:-${dly}s;border-color:${color}"></div>`;
  }
...
function renderTimeOnRoad() {
  ...
  const tankEl = document.getElementById('tor-fun');

  if (tripsWithDuration.length === 0) {
    document.getElementById('tor-total').textContent = '0h 0m';
    document.getElementById('tor-avg').textContent = '–';
    tankEl.dataset.cachedMins = '';
    tankEl.innerHTML = `<div style="font-size:0.72rem;color:var(--muted);padding:6px 0">Log a trip with its duration to start filling the tank — every hour on the road adds a fish.</div>`;
    return;
  }
  ...
  // Only rebuild when total minutes changes — keeps fish animations smooth
  const cached = tankEl.dataset.cachedMins;
  if (cached && parseInt(cached) === totalMins) return;
  tankEl.dataset.cachedMins = totalMins;

  const { fishCount, isDying, cycleNumber } = getFishCycleState(totalMins);
  const caption = getFishCaption(fishCount, isDying, cycleNumber);

  tankEl.innerHTML = `
    <div class="fish-tank">${renderFishTank(totalMins)}</div>
    <div class="fish-tank-caption">
      <span class="fish-count">${fishCount} fish</span>
      <span class="fish-comment">${caption}</span>
    </div>
  `;
}
...
function getDirectionMixForDow(dow) {
  // dow matches JS Date.getDay(): 0=Sun, 1=Mon, ... 6=Sat
  const HAILING = ['Bolt', 'Uber', 'InDrive'];
  const dayTrips = state.trips.filter(t => HAILING.includes(t.mode) && new Date(t.date).getDay() === dow);
  const toAttwell = dayTrips.filter(t => t.route === 'Rissik → Attwell' || t.route === 'University → Attwell').length;
  const toRissik  = dayTrips.filter(t => t.route === 'Attwell → Rissik' || t.route === 'Attwell → University').length;
  const total = toAttwell + toRissik;
  if (total < 3) return null; // not enough history for this specific day yet
  return { attwellShare: toAttwell / total, rissikShare: toRissik / total, samples: total };
}

function getExpectedFareForDow(dow) {
  const mix = getDirectionMixForDow(dow);
  if (!mix) return (state.fares.boltAttwell + state.fares.boltRissik) / 2; // cold-start fallback
  return mix.attwellShare * state.fares.boltAttwell + mix.rissikShare * state.fares.boltRissik;
}   
...
function getWeeklyBoltCost() {
  ...
  return days.reduce((sum, d) => sum + d.count * getExpectedFareForDow(d.dow), 0);
}
...
function getRemainingProjection() {
  ...
  const extra = state.extraDays.reduce((s,d) => {
    if (d.date >= new Date().toISOString().split('T')[0]) {
      const fare = getExpectedFareForDow(new Date(d.date).getDay());
      if (d.mode === 'Bolt + Rea Vaya') return s + fare + state.fares.reaVaya*2;
      if (d.mode === 'Bolt only') return s + fare;
...
// ─── GITHUB BACKUP ───────────────────────────────────────────────────────────
function getGithubToken() { return localStorage.getItem('transitGithubToken') || ''; }

function saveGithubToken() {
  const val = document.getElementById('gh-token-input').value.trim();
  if (!val) { showToast('Paste your token first'); return; }
  localStorage.setItem('transitGithubToken', val);
  document.getElementById('gh-token-input').value = '';
  document.getElementById('gh-status').textContent = 'Token saved ✓';
  showToast('GitHub token saved ✓');
}

function clearGithubToken() {
  if (!confirm('Remove the saved GitHub token?')) return;
  localStorage.removeItem('transitGithubToken');
  document.getElementById('gh-status').textContent = 'Token removed';
  showToast('GitHub token removed');
}

async function backupToGithub(wipeAfter) {
  const token = getGithubToken();
  const repo = document.getElementById('gh-repo').value.trim();
  const path = document.getElementById('gh-path').value.trim() || 'transit-backup.json';
  ...
  try {
    ...
    if (getRes.status === 200) {
      ...
      throw new Error(`Could not check existing file (${getRes.status})`);
    }
    ...
    const putRes = await fetch(apiUrl, {
      ...
      body: JSON.stringify({
        message: `Transit backup — ${new Date().toISOString()}`,
        content: b64,
        ...(sha ? { sha } : {})
      })
    });
    ...
    statusEl.textContent = `Backed up to ${repo}/${path} ✓`;
    showToast('Backed up to GitHub ✓');

    if (wipeAfter) {
      if (!confirm('Backup succeeded. Wipe ALL local data now? This resets trips, fares, and settings to defaults and cannot be undone locally — only your GitHub backup will remain.')) return;
      state = JSON.parse(JSON.stringify(DEFAULT_STATE));
      saveState();
      renderDashboard();
      statusEl.textContent = 'Wiped locally. Backup is safe on GitHub.';
      showToast('Local data wiped ✓');
    }
  } catch (e) {
    statusEl.textContent = 'Backup failed: ' + e.message;
    showToast('GitHub backup failed');
  }
}
...
const PIN_FAIL_KEY = 'transitPinFails'; // { count, lockUntil } — in localStorage, survives reload/restart
const PIN_CONTACT_KEY = 'transitPinContact';
...
function getPinContact() { return localStorage.getItem(PIN_CONTACT_KEY) || ''; }

function savePinContact(val) { localStorage.setItem(PIN_CONTACT_KEY, val); }

function renderPinContactOnOverlay() {
  const el = document.getElementById('pin-contact-msg');
  if (!el) return;
  const msg = getPinContact();
  el.textContent = msg ? `If found, please contact: ${msg}` : '';
}

function savePinContactFromInput() {
  const val = document.getElementById('pin-contact-input').value.trim();
  savePinContact(val);
  showToast(val ? 'Contact message saved ✓' : 'Contact message cleared');
}
...
function savePinFailState(s) { localStorage.setItem(PIN_FAIL_KEY, JSON.stringify(s)); }

function getLockoutSeconds(failCount) {
  if (failCount < 3) return 0;
  if (failCount < 5) return 30;
  if (failCount < 8) return 120;
  return 600; // caps at 10 min rather than escalating forever
}
...
function checkPinLockout() {
  ...
  const now = Date.now();
  if (s.lockUntil > now) {
    const secsLeft = Math.ceil((s.lockUntil - now) / 1000);
    document.getElementById('pin-error').textContent = `Too many attempts — wait ${secsLeft}s`;
...
async function processPinEntry() {
  if (pinMode === 'enter') {
    if (checkPinLockout()) { resetPinBuffer(); return; }
    const hash = await sha256(pinBuffer);
    const stored = localStorage.getItem(PIN_KEY);
    if (hash === stored) {
      savePinFailState({ count: 0, lockUntil: 0 });
      markUnlocked();
      hidePinOverlay();
      ...
      const s = getPinFailState();
      s.count += 1;
      const lockSecs = getLockoutSeconds(s.count);
      s.lockUntil = lockSecs > 0 ? Date.now() + lockSecs * 1000 : 0;
      savePinFailState(s);
      document.getElementById('pin-error').textContent = lockSecs > 0 ? `Incorrect PIN — locked ${lockSecs}s` : 'Incorrect PIN';
      pinDotsError();
    }
...
function showPinOverlay(mode) {
  ...
  if (pinMode === 'enter') checkPinLockout();
}
...
function initPinLock() {
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden && getPinCfg().enabled && hasPinHash() && isUnlocked()) lockApp();
  });
  window.addEventListener('pagehide', () => {
    if (getPinCfg().enabled && hasPinHash() && isUnlocked()) lockApp();
  });
}
...
function renderPinSettings() {
  ...
  el.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      <div>
        <label class="form-label">"If found" contact message</label>
        <input type="text" id="pin-contact-input" placeholder="e.g. John, +27 71 234 5678" value="${getPinContact()}">
        <button class="btn btn-secondary" style="margin-top:8px;font-size:0.72rem;padding:8px 12px" onclick="savePinContactFromInput()">Save message</button>
      </div>
...
(function() {
  ...
  const manifest = {
    ...
    icons: [
      {
        src: "data:image/svg+xml,%3Csvg xmlns='http:%2F%2Fwww.w3.org%2F2000%2Fsvg' viewBox='0 0 512 512'%3E%3Crect width='512' height='512' fill='%230f0f0f'%2F%3E%3Cpath d='M256,120 C200,120 156,164 156,220 C156,280 256,360 256,360 C256,360 356,280 356,220 C356,164 312,120 256,120 Z' fill='%23c8f135'%2F%3E%3Ccircle cx='256' cy='212' r='34' fill='%230f0f0f'%2F%3E%3Ccircle cx='256' cy='382' r='9' fill='%2335f1c8'%2F%3E%3Ccircle cx='292' cy='396' r='6.5' fill='%2335f1c8' opacity='0.75'%2F%3E%3Ccircle cx='322' cy='400' r='4' fill='%2335f1c8' opacity='0.5'%2F%3E%3C%2Fsvg%3E",
        sizes: '512x512',
        type: 'image/svg+xml',
